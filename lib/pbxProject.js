...
pbxProject.prototype.pbxXCConfigurationListByKey = function(key) {
    return this.pbxXCConfigurationList()[key];
};
...
pbxProject.prototype.addTarget = function(name, type, subfolder) {
    ...
    if (!producttypeForTargettype(targetType)) {
        var nextTargetType = targettypeForPointindentifier(targetType);
        if (!nextTargetType) throw new Error("Target type invalid: " + targetType);
        targetType = nextTargetType;
    }

    // does this apply to more than just extensions?
    var targetPList = (targetType == 'app_extension') ? "Info.plist" : targetSubfolder + "-Info.plist";
    ...
    var buildConfigurationsList = [
        {
            ...
            buildSettings: {
                ...
                INFOPLIST_FILE: '"' + path.join(targetSubfolder, targetPList + '"'),
...
function targettypeForPointindentifier (pointIndentifier) {
    TARGETTYPE_BY_POINTIDENTIFIER = {
        'com.apple.services': 'app_extension',
        'com.apple.ui-services': 'app_extension',
        'com.apple.broadcast-services': 'app_extension',
        'com.apple.fileprovider-ui': 'app_extension',
        'com.apple.fileprovider-nonui': 'app_extension',
        'com.apple.keyboard-service': 'app_extension',
        //'com.apple.message-payload-provider': '', todo: determine if imessage or sticker-pack
        'com.apple.photo-editing': 'app_extension',
        'com.apple.share-services': 'app_extension',
        'com.apple.watchkit': 'watch_extension',
        'com.apple.widget-extension': 'app_extension'
    };

    return TARGETTYPE_BY_POINTIDENTIFIER[pointIndentifier];
}
...
function producttypeForTargettype (targetType) {
    ...
    PRODUCTTYPE_BY_TARGETTYPE = {
            ...
            watch_extension2: 'com.apple.product-type.watchkit2-extension',
            messages_app: 'com.apple.product-type.application.messages',
            messages_extension: 'com.apple.product-type.app-extension.messages',
            sticker_extension: 'com.apple.product-type.app-extension.messages-sticker-pack'
...
pbxProject.prototype.getTargetAttributes = function(target) {
    var attributes = this.getFirstProject()['firstProject']['attributes'];
    target = target || this.getFirstTarget();

    return (attributes['TargetAttributes'] &&
        attributes['TargetAttributes'][target.uuid]) || {};
};

pbxProject.prototype.getTargetAttribute = function(prop, target) {
    var attributes = this.getTargetAttributes(target);

    if (attributes) return attributes[prop];
};

pbxProject.prototype.addTargetAttributes = function(value, target) {
    var attributes = this.getFirstProject()['firstProject']['attributes'];
    if (attributes['TargetAttributes'] === undefined) {
        attributes['TargetAttributes'] = {};
    }
    target = target || this.getFirstTarget();
    attributes['TargetAttributes'][target.uuid] = value || {};
};
...
pbxProject.prototype.removeTargetAttributes = function(target) {
    var attributes = this.getFirstProject()['firstProject']['attributes'];
    target = target || this.getFirstTarget();
    if (attributes['TargetAttributes'] &&
        attributes['TargetAttributes'][target.uuid]) {
        delete attributes['TargetAttributes'][target.uuid];
    }
};
